...
fn build_with_artifacts_in_dir(path: &Path) {
    ...
    println!("cargo:rustc-env=DEFAULT_PYTHON_CONFIG_RS={}", config_path.display());
}
...
fn main() {
    if std::env::var("CARGO_FEATURE_BUILD_MODE_STANDALONE").is_ok() {
        ...
        std::fs::write(&path, DEFAULT_PYTHON_CONFIG.as_bytes()).expect("failed to write default python config");
        println!("cargo:rustc-env=DEFAULT_PYTHON_CONFIG_RS={}", path.display());
    ...
    if target_family == "windows" {
        embed_resource::compile("coolerod-manifest.rc");
    }
}
